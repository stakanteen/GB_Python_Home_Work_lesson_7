...
class Cell:
    ...
    def __sub__(self, other):
        sub_res = self.n - other.n
        if sub_res > 0:
            answer = sub_res
        else:
            answer = 'Результат меньше нуля'
        return answer
    ...
    def make_order(self, n_in_row):
        result = ''
        rows_in_n = self.n // n_in_row
        for i in range(rows_in_n):
            result += '*' * n_in_row + '\n'
        result += '*' * (self.n - n_in_row*rows_in_n) + '\n'
        return result
